...
//
// The bundle calendar is now LIVE. As of PR #13 (fix/bundle-adapter-and-mock-data,
// merged 2026-05-27) data.js no longer hardcodes `ND.todayISO = '2026-05-24'` —
// it sets `ND.todayISO = iso(new Date())` (data.js) and tabs.js re-pins the
// active day to today at the top of every render. The visible week is therefore
// the real-world week that contains today, and the active DAY is today.
//
// Fixtures must track that live anchor, so these dates are computed at run time:
//   - BUNDLE_TODAY   = the runner's local calendar day (matches the bundle's iso()).
//   - BUNDLE_SIBLING = a different day in the SAME Mon–Sun week, so its deals sit
//     in the visible week but on a non-active day (only shown after clicking that
//     day's tab — see F13). Weeks are Mon–Sun, so yesterday is same-week except
//     when today is Monday, in which case we use tomorrow.
//
// Computing these (rather than hardcoding) means the suite never silently rots
// when the wall clock advances — the failure mode that broke the old frozen
// 2026-05-24 constants once the bundle went live.
function localYMD(d) {
...
const SIBLING_DATE = (() => {
  ...
  // getDay(): 0=Sun … 1=Mon. Monday's "yesterday" crosses into the prior
  // Mon–Sun week, so step forward instead; every other day steps back.
  d.setDate(TODAY_DATE.getDate() + (TODAY_DATE.getDay() === 1 ? 1 : -1));
  return d;
})();
const BUNDLE_TODAY = localYMD(TODAY_DATE);
const BUNDLE_SIBLING = localYMD(SIBLING_DATE);
...
test.describe('Deal Feed Excel — PRD flow coverage (Stage 2 — data-bound flows)', () => {
  ...
  // F12 — Expand row → "Open Deal Room" → /deal/:id navigation.
  // PRD intent: the user can reach the full deal detail page (/deal/<id>) from
  //       the grid. Bundle reality after PR #13 (d788868): dbl-click on the row
  //       gutter toggles the INLINE EXPAND row (ND._toggleExpand) — the original
  //       Deal Feed Excel design behavior, restored after it was inadvertently
  //       changed to direct openDetail in PR #6. Navigation to /deal/:id now
  //       happens via the "Open Deal Room" link (<a class="act-btn primary"
  //       href="/deal/:id">) rendered inside that expanded row (feed.js).
  //       This test exercises the real two-step gesture end to end.
  test('F12: expand row then Open Deal Room link navigates to /deal/:id', async ({ page }) => {
    await authAndOpenSheet(page, { deals: buildFixtureDeals() });
    const shell = page.locator('.nd-excel-shell');

    // Dbl-click the gutter cell to open the inline expand row (no longer navigates).
    await shell.locator('tr.dr[data-id="deal-1"] td.gutter').dblclick();
    await expect(shell.locator('tr.xr')).toHaveCount(1);

    // The expanded row hosts the "Open Deal Room" link → React Router /deal/:id.
    const openLink = shell.locator('tr.xr a.act-btn.primary', { hasText: 'Open Deal Room' });
